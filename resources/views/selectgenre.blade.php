   ...
   @if($moviegenreitems != NULL && count($moviegenreitems)>0)
     <h5 class="section-heading">{{$genre->genre->name }} in {{ $menu->name }} </h5>
       ...
       @if($auth && getSubscription()->getData()->subscribed == true)
         ...
         <a href="{{ route('show.in.genre',$genre->genre->id) }}" class="see-more"> <b>{{__('View All')}}</b></a>
     
       @else
       
         <a href="{{ route('show.in.guest.genre',$genre->genre->id) }}" class="see-more"> <b>{{__('View All')}}</b></a>
                               ...
                               @if($auth && getSubscription()->getData()->subscribed == true)
                                 @if (isset($gets1->episodes[0]) && checkInTvseries($item) == true && isset($gets1->episodes[0]->video_link))
                                   @if($item->age_req == 'all age' || $age>=str_replace('+', '', $item->age_req))
                                     ...
                                     @else
                                       <a href="{{ route('watchTvShow',$item->seasonid) }}" class="iframe btn btn-play"><span class="play-btn-icon"><i class="fa fa-play"></i></span> <span class="play-text">{{__('Play Now')}}</span></a>
                                     @endif
                                   @else
