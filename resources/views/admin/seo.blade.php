    ...
    <div class="card m-b-30">
      <div class="card-body ml-2">
        <div class="card-body bg-success-rgba">
          <div class="row align-items-center">
            <div class="col-12">
              <h5 class="text-success process-fonts"><i class="fa fa-info-circle"></i> {{ __('SITE MAP SETTTINGS') }}</h5>
                {!! Form::label('sitemap', __('Generate Sitemap')) !!}
                <br>
                <a href="{{ url('/sitemap') }}" class="btn btn-primary-rgba">{{ __('Generate') }}</a>
                @if(@file_get_contents(public_path().'/sitemap.xml'))
                  <a href="{{ url('/sitemap/download') }}" class="btn btn-primary-rgba" data-toggle="tooltip" data-original-title="{{__('download sitemap.xml')}}"><i class="material-icons">download</i>Sitemap.xml</a>
                  |
                  <a href="{{ url('/sitemap.xml') }}" class="btn btn-primary-rgba" data-toggle="tooltip" data-original-title="{{__('view sitemap')}}"><i class="material-icons">visibility</i>Sitemap</a>
                @endif
              </div>
            </div>
          </div>
