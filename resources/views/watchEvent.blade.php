...
<head>
	<title>{{__('Watch Movie')}} - {{ $liveevent->title }}</title>
	 ...
	 $(document).ready(function(){
		...
		for(var i=0; i<tt; i++){
			video = FWDUVPlayer.instaces_ar[i];
			 ...
			 $.ajax({
            type: "get",
            url: SITEURL + "/user/event/time/"+video['curTime']+'/'+event_id+'/'+user_id,
...
<body style="background-color:#999999; padding:0px; margin:0px;">	
	...
	<ul id="playlists" style="display:none;">
		...
		<li data-source="trailer"
		 data-playlist-name="{{ $liveevent->title }}" data-thumbnail-path="{{asset('images/events/thumbnails/'.$liveevent->thumbnail)}}" >
			<p class="minimalDarkCategoriesTitle"><span class="minimialDarkBold">Title: {{ $liveevent->title }}</span></p>
			<p class="minimalDarkCategoriesDescription"><span class="minimialDarkBold">Description: </span>{{ $liveevent->detail }}</p>
		</li>
	...
	<ul id="trailer">
		...
		@endif
		data-thumb-source="{{asset('images/events/thumbnails/'.$liveevent->thumbnail)}}" 
		
	
		
		data-video-source="{{ ($liveevent->ready_url) }}"
		> 
		...
		<ul data-ads="">
			<li @if($skipad->ad_video !="no")
				...
				@else
				data-source="{{ $skipad->ad_url }}" @endif data-time-start="{{ $skipad->time }}" data-time-to-hold-ads="{{ $skipad->ad_hold }}"data-thumbnail-source="{{asset('images/events/thumbnails/'.$liveevent->thumbnail)}}" data-link="{{ $skipad->ad_target }}" data-target="_blank"></li>
			</ul>
			@endif

			<div data-video-short-description="">
				<p class="minimalDarkCategoriesTitle"><span class="minimialDarkBold">{{__('Title')}}: </span>{{ $liveevent->title }}</p>
				<p class="minimalDarkCategoriesDescription"><span class="minimialDarkBold">{{__('Description')}}: </span>{{ $liveevent->detail }}</p>
			</div>
			<div>
				<p class="classicDarkThumbnailTitle">{{__('title')}}: </span>{{ $liveevent->title }}</p>
				<p class="minimalDarkThumbnailDesc">{{__('description')}}: </span>{{ $liveevent->detail }}</p>
			</div>
