...
<head>
	<title>{{__('Watch')}}  - {{ $season['tvseries']['title'] }}</title>
	 	...
	 	$(document).ready(function(){
			...
			setInterval(function(){
				...
				for(var i=0; i<tt; i++){
					...
					$.ajax({
						type: "get",
						url: SITEURL + "/user/tv/time/"+video['curTime']+'/'+tv_id+'/'+user_id +'/'+video['totalTime'],
						data:{
	...
	<script type="text/javascript">
		FWDUVPUtils.onReady(function(){
			...
			new FWDUVPlayer({		
					...
					thumbnailNormalBackgroundColor:"#272727",
					thumbnailHoverBackgroundColor:"#FFFFFF",
					thumbnailDisabledBackgroundColor:"#D3D3D3",
...
<body style="background-color:#999999; padding:0px; margin:0px;">	
	...
	<ul id="playlists" style="display:none;">
		...
		<li data-source="tvshow" data-playlist-name="{{ $season['tvseries']['title'] }}" data-thumbnail-path="{{asset('images/tvseries/thumbnails/'.$season['tvseries']['thumbnail'])}}">
			...
			<p class="minimalDarkCategoriesDescription"><span class="minimialDarkBold">{{__('description')}}: </span>{{ $season['tvseries']['detail'] }}</p>
		</li>
	...
	<ul id="tvshow">
		...
		if($cpy->is_resume == 1){
			$filename = 'time.json';
			if(file_exists(storage_path() .'/app/time/tv/user_'.$user_id.'/episode_'.$episod.'/'.$filename)){
				$result = @file_get_contents(storage_path() .'/app/time/tv/user_'.$user_id.'/episode_'.$episod.'/'.$filename);
		...
		<li class="episodes" @if($pauseads->count()>0 && (isset($remove_ads) && $remove_ads != 1))
			...
			<div data-video-short-description="">
				<p class="minimalDarkCategoriesTitle"><span class="minimialDarkBold">{{__('title')}}: </span>{{ $epi->title }}</p>
				
				<p class="minimalDarkCategoriesDescription"><span class="minimialDarkBold">{{__('description')}}: </span>{{ $epi->detail }}</p>
			</div>
