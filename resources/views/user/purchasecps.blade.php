...
@section('main-wrapper')
  ...
  <section id="main-wrapper" class="main-wrapper home-page user-account-section">
    <div class="container-fluid">
      <h4 class="heading">{{__('complete your payment')}}</h4>
      <ul class="bradcump">
          ...
          <div class="card" style="border: 1px solid #000; border-radius: 5px; background-color:#fff;">
            <div class="text-center push">
                ...
                <div style="background-color:#333; color:#fff;">
                    <div>{{__('you have to send')}}</div>
                    <span class="h3">{{$tinfo["amount2"]}}</span>
                    <div >{{$tinfo["currency2"]}}</div>
                </div><br/>
                <small class="font-size-sm text-muted" >{{$etinfo["recv_confirms"]}} / {{$tinfo["confirms_needed"]}} {{__('confirmation')}}</small>
            </div>
            <div class="" style="padding: 5px;">
              <div class="form-group">
                  <label for="pmaddress">{{$tinfo["currency2"]}} {{__('address')}}:</label>
                  ...
                  <small class="form-text text-muted text-center"><a href="#" onclick="CopyOnClipboardBtcAddress()"><i class="fa fa-copy"></i> {{__('copy on clipboard')}}</a></small>
              </div>
              <div class="form-group row m-t-20">
                  <div class="col-sm-6 text-left"><button class="btn btn-primary" type="button" data-toggle="modal" data-target="#QrCodeBtc"><i class="fa fa-qrcode"></i> {{__('qrcode')}}</button></div>
                  <div class="col-sm-6 text-right"><a class="btn btn-primary" href="{{$tinfo['status_url']}}" target="_blank"><i class="fa fa-tasks"></i> {{__('payment status')}}</a></div>
              </div>
            </div>
          </div>
          <!---->
  ...
  <div class="modal" id="QrCodeBtc">
      <div class="modal-dialog" style="width: 246px; z-index: 99999;">
          <div class="modal-content">
              ...
              <button type="button" class="btn btn-block btn-danger" data-dismiss="modal"><i class="fa fa-times"></i> {{__('close')}}</button>
          </div>
      </div>
  </div>
