...
class HomeController extends Controller
{
    ...
    public function showallsinglemovies(Request $request)
    {
        //$ipaddress = $request->getClientIp();
        $ipaddress='43.251.92.73'; 
        $geoip = geoip()->getLocation($ipaddress);
        ...
        $movies = Movie::orderBy('id', 'DESC')->where('status', '=', 1)->where('country', 'NOT like', '%'.$usercountry.'%')->paginate(30);
        $ad = $this->ad;
        $age = 0;
        ...
        return view('viewall2', compact('movies', 'catlog', 'ad','age'));
    ...
    public function showallsingletvseries(Request $request)
    {
        ...
        $all_tvseries = TvSeries::where('status', '=', 1)->where('country', 'NOT like', '%'.$usercountry.'%')->get();
    ...
    public function guestindex(Request $request, $menu_slug)
    {
        ...
        if ($this->configs->prime_genre_slider == 1) {
            //Layout 1
            return view('home', compact('request','getallgenre','protip', 'menu_data', 'recent_data', 'ad', 'age', 'genres', 'menuh', 'catlog', 'withlogin', 'menu', 'subscribed', 'audiolanguages', 'section6', 'blogs', 'liveevent', 'home_slides', 'age', 'audio', 'short_promo', 'audio_lang', 'upcomingitems', 'getallaudiolanguage', 'top_data'));
        } else {
            //Layout 2
            return view('home2', compact('request','getallgenre','protip', 'menu_data', 'recent_data', 'ad', 'age', 'genres', 'menuh', 'catlog', 'withlogin', 'menu', 'subscribed', 'audiolanguages', 'section6', 'blogs', 'liveevent', 'home_slides', 'age', 'audio', 'short_promo', 'audio_lang', 'upcomingitems', 'getallaudiolanguage', 'top_data'));
        }
    ...
    public function index(Request $request, $menu_slug)
    {
        ...
        $menu_data = MenuVideo::where('menu_id', $menu->id)->get();
        $recent_data = DB::table('menu_videos')->where('menu_id', $menu->id)->orderBy('id', 'DESC')->get();
        ...
        $audiolanguages = AudioLanguage::select('id', 'language')->paginate(10);
        //$section6 = MenuSection::where('section_id', '=', 6)->where('menu_id', '=', $menu->id)->first();
        $section = MenuSection::where('menu_id', '=', $menu->id)->orderBy('position', 'ASC')->get();
        //return($section);
        $top_data = Menu::whereSlug($menu_slug)
            ->whereHas('menu_data')
            ->whereHas('menusections')
        ...
        elseif ($this->configs->prime_genre_slider == 1) {
            //Layout 1
            //return $usercountry;
            return view('home', compact('request','menu_data', 'recent_data', 'home_slides', 'ad', 'age', 'genres', 'menuh', 'catlog', 'withlogin', 'menu', 'subscribed', 'watchistory', 'audiolanguages',  'getallgenre', 'blogs', 'liveevent', 'protip', 'audio', 'short_promo', 'audio_lang', 'upcomingitems', 'getallaudiolanguage', 'top_data'));
        } else {
            //Layout 2
            return view('home2', compact('request','menu_data', 'recent_data', 'home_slides', 'ad', 'age', 'genres', 'menuh', 'catlog', 'withlogin', 'menu', 'subscribed', 'watchistory', 'audiolanguages',  'getallgenre', 'blogs', 'liveevent', 'protip', 'audio', 'short_promo', 'audio_lang', 'upcomingitems', 'getallaudiolanguage', 'top_data'));
        }
    ...
    public function showallgenre(Request $request, $id)
    {
        ...
        if (isset($genre)) {
            $items = collect();
            // $movies = Movie::where('genre_id', $genre->id)->where('status', 1)->get();
            $movie_data = Movie::where('status', 1)->get();
           
           $movie_ids=[];
           foreach($movie_data as $movie){
            foreach(explode(',',$movie->genre_id) as $gid){
                if($gid==$genre->id){
           ...
           $movies = Movie::whereIn('id',$movie_ids)->where('status', 1)->get();
            ...
            foreach ($movies as $movie) {
             ...
             $tvs_data = TvSeries::where('status', 1)->get();
           
           $tvseries_ids=[];
           foreach($tvs_data as $tv){
            foreach(explode(',',$tv->genre_id) as $gtid){
                if($gtid==$genre->id){
                    array_push($tvseries_ids,$tv->id);
                }
            }
           }

             $tvs = TvSeries::whereIn('id',$tvseries_ids)->where('status', 1)->whereHas('seasons_first')->get();
    ...
    public function gusetshowallgenre(Request $request, $id)
    {
        ...
        if (isset($genre)) {
           ...
           $tv_ids=[];
           foreach($tvs_data as $tv){
            foreach(explode(',',$tv->genre_id) as $gid){
                if($gid==$genre->id){
                    array_push($tv_ids,$tv->id);
                }
            }
            ...
            $tvs = TvSeries::where('id',  $tv_ids)->where('status', 1)->whereHas('seasons_first')->get();
    ...
    public function guestshowallalang(Request $request, $id)
    {
        ...
        if (isset($alang)) {
            ...
            $movie_data = Movie::where('status', 1)->get();
             $movie_ids=[];
           foreach($movie_data as $movie){
            foreach(explode(',',$movie->a_language) as $gid){
                if($aid==$alang->id){
                    array_push($movie_ids,$movie->id);
                }
            }
